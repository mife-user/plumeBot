...
| B-010 | 画像缓存淘汰延迟配置化 | P3-002 实现 | 需要时（新增配置字段需双处同步，见 B-006） | `profileEvictDelay` 硬编码为 20 轮，未配置化；需调参时再下沉为配置字段 |
| B-011 | 画像渐进式更新未规划 | P3-002 验收边界 | P3-004 后评估 | 活跃度/亲密度等统计数据的渐进式维护（架构 §4.2/§4.4 声明由规则层自动维护）暂无任务承接；P3-002 仅实现加载/缓存 |
